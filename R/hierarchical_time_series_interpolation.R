...
legend("topright", legend = 1:n, col = 1:n, lty = 1, title = "y")
...
library("ggplot2")
x10 <- sample_df %>% filter(level == 'x10')
ggplot(x10, aes(x = t, y = y_p)) +
  geom_line(aes(y=y_p,col="blue", alpha=1)) +
  geom_point(aes(y=value,col="black", alpha=1)) +
  labs(x = "Time", y = "y", title = "Time Series #10")
